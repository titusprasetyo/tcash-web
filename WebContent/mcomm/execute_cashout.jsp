	...
	if(user != null)
	{
   	//	session.putValue("user", user);
		...
		try
		{
			...
			String q = "SELECT AMOUNT, MERCHANT_ID FROM MERCHANT_CASHOUT WHERE CASHOUT_ID=? AND IS_EXECUTED = 0";
			...
			if (rs!=null && rs.next()) {
				...
				if (retval[0].equals("00")) {
					...
					ps.setString(2, "Execute Cashout|" + cashout_id);
					...
					ps = conn.prepareStatement(sql);
					ps.clearParameters();
					ps.setString(1, doc_number);
					ps.setString(2, cashout_id);
					ps.executeUpdate();
					//stmt.close();
					b1 = true;
			...
			if(e.toString().contains("executed")){
				b4=false;
				b5=true;
			}
		} finally{
		...
		if(b5) response.sendRedirect("execute_cashout.jsp?msg=Cashout Telah Di Eksekusi Oleh User Lain");
