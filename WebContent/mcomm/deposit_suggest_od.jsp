...
String insertDeposit(Connection con, String merchant_id, String amount, String doc_number, String note, String login, String ip_address)throws SQLException{
	...
	String sql = "";
	try{
		if(doc_number.length()>50){
			doc_number = doc_number.substring(0,49);
		}
		//System.out.println("doc_number="+doc_number.length());
		sql = "insert into MERCHANT_DEPOSIT (MERCHANT_ID, AMOUNT, DOC_NUMBER, NOTE, DEPOSIT_TIME, EXEC_TIME, IS_EXECUTED, ENTRY_LOGIN) VALUES ('"+merchant_id+"',"+amount+",'"+doc_number+"','"+note+"',sysdate,sysdate,'1','"+login+"')";
...
String updateAccount(Connection con, String merchant_id, String tx_date, String tipe, String description, String amount)throws SQLException{
	...
	try{
		...
		sql = "UPDATE ACC_STATEMENT SET REF='"+merchant_id+"' WHERE TX_DATE=TO_DATE('"+tx_date.substring(0, tx_date.length()-2)+"', 'YYYY-MM-DD HH24:MI:SS') AND TIPE='"+tipe+"' AND DESCRIPTION='"+description+"' AND AMOUNT="+amount;
		pstmt = con.prepareStatement(sql);
		pstmt.executeUpdate();
		pstmt.close();
		//System.out.println(sql);
...
<%!
String findMerchantName(Connection con, String merchant_id)throws SQLException{
	String ret = "99;Error";
	PreparedStatement pstmt = null;
	ResultSet rs = null;
	String sql = "";
	String merchant_name = "unknown";
	
	try{
		sql = "select name from merchant_info where merchant_info_id = (select merchant_info_id from merchant where merchant_id='"+merchant_id+"')";
		pstmt = con.prepareStatement(sql);
...
String deleteAccount(Connection con, String tx_date, String description, String amount, String tipe)throws SQLException{
	...
	try{
		sql = "UPDATE ACC_STATEMENT SET STATUS='99' WHERE TX_DATE=TO_DATE('"+tx_date.substring(0,tx_date.length()-2)+"', 'YYYY-MM-DD HH24:MI:SS') AND TIPE='"+tipe+"' AND DESCRIPTION='"+description+"' AND AMOUNT="+amount;
		pstmt = con.prepareStatement(sql);
		pstmt.executeUpdate();
		pstmt.close();
		//System.out.println(sql);
...
String findVA(Connection con, String VA) throws SQLException{
	...
	try{
		sql = "select merchant_id, merchant_name as VAname from merchant_virtual_id where merchant_virtual_id='"+(VA.substring(8, VA.length())).trim()+"' and status=1";
		//System.out.println(sql);
		pstmt = con.prepareStatement(sql);
		rs = pstmt.executeQuery();            
		if(rs.next()){
...
<stripes:layout-render name="/web-starter/layout/standard.jsp" title="Suggest Merchant Deposit">
	<stripes:layout-component name="contents">
		...
		<a href='../new_uploadDepositFile.jsp?idLog1=<%=encLogin%>&idLog2=<%=encPass%>'>Upload Deposit File</a>
		<br /><br />
        <%
        try{
	   ...
	   <%
            ...
            if(comm!=null&&fileID1!=null&&fileID2!=null&&fileID3!=null&&depositID1!=null&&depositID2!=null&&depositID3!=null){
				try{
					...
					for(int i=0;i<depositID1.length;i++){						
						String [] resultIns = (insertDeposit(con, depositID1[i], fileID3[i], "Virtual Account "+depositID3[i]+" Merchant "+depositID2[i], "VA "+fileID1[i].substring(0,10), encLogin, request.getRemoteAddr())).split(";");
        ...
        <%
            ...
            pstmt = con.prepareStatement("SELECT COUNT(*) AS jmlFile FROM ACC_STATEMENT WHERE STATUS='0' AND (tx_date between sysdate-14 and sysdate) AND TIPE='C' AND REF IS NULL");
            rs = pstmt.executeQuery();            
            while(rs.next()){
			...
			pstmt = con.prepareStatement("SELECT COUNT(*) AS jmlRecord FROM MERCHANT_DEPOSIT WHERE IS_EXECUTED='1' and (deposit_time between sysdate-14 and sysdate) AND exec_time is not null AND NOT DEPOSIT_ID IN (SELECT REF FROM ACC_STATEMENT WHERE NOT REF IS NULL)");
			rs = pstmt.executeQuery();            
            while(rs.next()){
			...
			pstmt = con.prepareStatement("SELECT COUNT(*) AS jmlFileVA FROM ACC_STATEMENT WHERE STATUS='0' AND UPPER(DESCRIPTION) like '%988911%' AND (tx_date between sysdate-14 and sysdate) and TIPE='C' AND REF IS NULL");
            rs = pstmt.executeQuery();            
            while(rs.next()){
			...
			pstmt = con.prepareStatement("SELECT TX_DATE, DESCRIPTION, AMOUNT FROM ACC_STATEMENT WHERE STATUS='0' AND UPPER(DESCRIPTION) like '%988911%' AND (tx_date between sysdate-14 and sysdate) and TIPE='C' AND REF IS NULL");
			rs = pstmt.executeQuery();            
			while(rs.next()){
			...
			%>
			<hr />
				<form action='deposit_suggest_od.jsp' method='post'>
				...
				<tr>
					...
					<%
					if(bolAction){
						//System.out.println("DEBUG DEBUG1.3");
					%>
					...
					noIndex++;
					if(bolAction){
						//System.out.println("DEBUG DEBUG1.4");
						jmlAction++;
						%>
				...
				<%
				//System.out.println("DEBUG DEBUG3");
			//cek kalau ada request deposit atau account statement yang akan dibandingkan                  
                ...
                pstmt = con.prepareStatement("SELECT TX_DATE, DESCRIPTION, AMOUNT FROM ACC_STATEMENT WHERE STATUS='0' AND (tx_date between sysdate-14 and sysdate) and TIPE='C' AND REF IS NULL");
                rs = pstmt.executeQuery();            
                while(rs.next()){
				...
				pstmt = con.prepareStatement("SELECT DEPOSIT_TIME, MERCHANT_ID, AMOUNT, DEPOSIT_ID FROM MERCHANT_DEPOSIT WHERE IS_EXECUTED='1' and (deposit_time between sysdate-14 and sysdate) and exec_time is not null AND NOT DEPOSIT_ID IN (SELECT REF FROM ACC_STATEMENT WHERE NOT REF IS NULL)");
				rs = pstmt.executeQuery();            
                while(rs.next()){
		...
		// rdp859
		// fix: fileIdUnMatch 100 --> 1000; fixed 20150701

                int [] idMatch = new int[50], idUnMatch = new int[50], fileIdMatch = new int[50], fileIdUnMatch = new int[1000];
				...
				// rdp859
				// an error occured here... fixed 20150701
				for(int i=0;i<jmlFileInt;i++){
				...
				<hr />
					<table width='45%' border='1' cellspacing='0' cellpadding='0' bordercolor='#FFF6EF'>
					<tr><td colspan='6'><div align='right'><font color='#CC6633' size='2' face='Verdana, Arial, Helvetica, sans-serif'><strong>.:: Unmatched Deposit List</strong></font></div></td></tr>
                ...
                if(idUnMatchInt!=0){
                    ...
                    for(int i=0;i<idUnMatchInt;i++){
						...
						String [] resultFind = (findMerchantName(con, matriksDeposit[idUnMatch[i]][1])).split(";");
						if(resultFind[0].equals("00"))merchantName = resultFind[1];
						%>
						<tr><td><div align='center'><font size='1' face='Verdana, Arial, Helvetica, sans-serif'><%=idUnMatch[i]+1%></font></div></td>
						...
						<td><div align='center'><font size='1' face='Verdana, Arial, Helvetica, sans-serif'><%=merchantName%></font></div></td>
				...
				%>
					...
					<td bgcolor='#CC6633'><div align='center'><font color='#FFFFFF' size='1' face='Verdana, Arial, Helvetica, sans-serif'><strong>Amount</strong></font></div></td>
					<td bgcolor='#CC6633'><div align='center'><font color='#FFFFFF' size='1' face='Verdana, Arial, Helvetica, sans-serif'><strong>Suggested Deposit (MID|MName|DID)</strong></font></div></td>
				...
				if(fileIdUnMatchInt!=0){
					for(int i=0;i<fileIdUnMatchInt;i++){
						...
						%>
						<tr>
						<td><div align='center'><font size='1' face='Verdana, Arial, Helvetica, sans-serif'><%=fileIdUnMatch[i]+1%></font></div></td>
						...
						for(int j=0;j<idUnMatchInt;j++) {
							String merchantName = "unknown";
							String [] resultFind = (findMerchantName(con, matriksDeposit[idUnMatch[j]][1])).split(";");
							if(resultFind[0].equals("00"))merchantName = resultFind[1];
							if((matriksFile[fileIdUnMatch[i]][2]).equalsIgnoreCase(matriksDeposit[idUnMatch[j]][2])){
							%><option value='<%=matriksDeposit[idUnMatch[j]][3]%>'><%=matriksDeposit[idUnMatch[j]][1]%>|<%=merchantName%>|<%=matriksDeposit[idUnMatch[j]][3]%></option><%
							ifexist &= false;
							}
						}
                        %>
						...
						<%if(!ifexist){
							%>*|<a href="javascript:doDeleteAccount('<%=(matriksFile[fileIdUnMatch[i]][0])%>','<%=(matriksFile[fileIdUnMatch[i]][1])%>','<%=(matriksFile[fileIdUnMatch[i]][2])%>','C');">-</a><%;
							}else{
